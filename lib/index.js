...
function fetchDeployHistory(siteId, accessToken, proxyUrl, maxDeploys = 10) {
  const url = proxyUrl ? `${proxyUrl}/sites/${siteId}/deploys` : `https://api.netlify.com/api/v1/sites/${siteId}/deploys`, headers = {};
  return accessToken && (headers.Authorization = `Bearer ${accessToken}`), jsonRequest(url, {
    method: "GET",
    headers
  }).pipe(
    operators.map(
      (deploys) => deploys.slice(0, maxDeploys).map((deploy2) => ({
        ...deploy2,
        buildTime: deploy2.publishedAt && deploy2.createdAt ? new Date(deploy2.publishedAt).getTime() - new Date(deploy2.createdAt).getTime() : void 0
      }))
    )
  );
}
const initialState = {
  ...
  ), refreshDeployHistory$ = rxjs.timer(0, pollIntervalMs).pipe(
    operators.switchMap(
      () => rxjs.of(configuredSites).pipe(
        operators.switchMap(
          (sites) => rxjs.merge(
            ...sites.map(
              (site) => fetchDeployHistory(site.id, accessToken, proxyUrl, maxDeploys).pipe(
                operators.map((deploys) => ({ type: "deployHistory/updated", siteId: site.id, deploys })),
                operators.catchError(() => rxjs.of({ type: "deployHistory/failed", siteId: site.id }))
              )
            )
          )
        )
  ...
  ), fastRefresh$ = rxjs.timer(0, fastPollIntervalMs).pipe(
    operators.switchMap(
      () => rxjs.of(configuredSites).pipe(
        operators.switchMap(
          (sites) => rxjs.merge(
            ...sites.map(
              (site) => fetchDeployHistory(site.id, accessToken, proxyUrl, maxDeploys).pipe(
                operators.map((deploys) => ({ type: "deployHistory/fastUpdated", siteId: site.id, deploys })),
                operators.catchError(() => rxjs.of({ type: "deployHistory/failed", siteId: site.id }))
              )
            )
          )
...
function NetlifyWidget(props) {
  const netlifySitesUrl = "https://app.netlify.com/account/sites", { title, description, isLoading, sites, onDeploy, deployHistory = {}, isRefreshing = !1 } = props;
