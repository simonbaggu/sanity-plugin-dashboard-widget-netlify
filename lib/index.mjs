...
import { map, catchError, scan, switchMap, debounceTime, distinctUntilChanged, startWith } from "rxjs/operators";
...
function tryCorsProxy(netlifyUrl, accessToken, proxyIndex = 0) {
  if (proxyIndex >= CORS_PROXIES.length)
    return console.warn("All CORS proxies failed. Deploy history unavailable."), of([]);
  ...
  return accessToken && (headers.Authorization = `Bearer ${accessToken}`), jsonRequest(url, {
    ...
    map((deploys) => Array.isArray(deploys) ? deploys.filter(
    ...
    catchError((error) => (console.warn(`CORS proxy ${proxyIndex + 1} failed:`, error.message), tryCorsProxy(netlifyUrl, accessToken, proxyIndex + 1)))
  );
}
...
const initialState = {
  ...
  ), refreshDeployHistory$ = timer(pollIntervalMs, pollIntervalMs).pipe(
    switchMap(
      () => of(configuredSites).pipe(
        switchMap(
          (sites) => createDeployHistoryStreams(
  ...
  ), fastRefresh$ = timer(fastPollIntervalMs, fastPollIntervalMs).pipe(
    switchMap(
      () => of(configuredSites).pipe(
        switchMap(
          (sites) => createDeployHistoryStreams(
